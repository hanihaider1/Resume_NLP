...
def get_similarity(resume_list,job_description_tokenised,holder):
  if holder==1:
    ...
    job_tokens = tokenizer.encode(job_description_tokenised, add_special_tokens=True, max_length=512, truncation=True, padding='max_length', return_attention_mask=True, return_tensors='pt')
    ...
    similarity_list = sorted(zip(resume_text, similarity_scores[0]), key=lambda x: x[1], reverse=True)
  elif holder==3:
    ...
    for selected_resume_pp in resume_list:
